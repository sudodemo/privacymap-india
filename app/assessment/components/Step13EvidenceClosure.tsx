...
useState<Record<string, EvidenceState>>({});

const decisionMap = useMemo(
() =>
new Map(
decisions.map((d) => [
`${d.riskTitle}::${d.category}`,
d,
])
),
[decisions]
...
(a) =>
evidence[a.id]?.verified
).length;

/*

* A closure item is ready only when:
* 1. treatment is Completed/Accepted
* 2. evidence has been verified
* 3. an evidence reference exists
* 4. governance approval is Approved
     */
     const closureReadyCount =
     eligible.filter((a) => {
     const e =
     evidence[a.id] || {
     reference: "",
  ...
  const d = decisionMap.get(
  `${a.riskTitle}::${a.category}`
  );
...
> <div style={card}> <div style={kicker}>
STEP 13 </div>
    ...
    <p style={p}>
      Confirm that completed or accepted
      remediation has supporting evidence
      and that governance approvals are
      recorded.
    </p>
        ...
        return (
          ...
          >
            ...
            >
              ...
              and verified that it supports
              the stated remediation.
            </label>
            ...
            {ready && (
              ...
              >
                ...
                Treatment, evidence and
                approved governance are all
                present.
              </div>
            )}
          </div>
  ...
  >
    ...
    Completed or Accepted does not by
    itself prove control effectiveness.
    Evidence and applicable governance
    approval should be reviewed before
    closure.
  </div>
</section>
...
>
No remediation action is currently
marked Completed or Accepted. Evidence
closure becomes available when treatment
reaches one of those statuses. </div>
);
}
