...
/** Remove non-printing control characters while preserving normal Unicode. */
...
/**
 * UI-safe text normalisation. This is intentionally conservative:
 * it does not HTML-decode or execute anything and React will escape
 * the returned value when rendered as text.
 */
...
/** Validate text without changing it. Throws on unsafe structural input. */
export function validateText(
  ...
  if (normalized.length > maxLength) {
    throw new Error(
      `${options.fieldName ?? "Input"} exceeds the maximum allowed length of ${maxLength} characters.`
    );
  }
...
/** Returns true only for ordinary HTTP(S) URLs. */
export function isSafeHttpUrl(value: unknown): value is string {
  if (typeof value !== "string" || !value.trim()) return false;
  if (JAVASCRIPT_SCHEME.test(value)) return false;
 ...
 * Excel/LibreOffice can interpret cells beginning with these characters
 * as formulas when a CSV is opened.
 */
...
/**
 * Security validation for imported assessment packages.
 * The package has already passed its structural/schema validation.
 * This second pass prevents oversized strings, control characters,
 * HTML-like markup and unsafe URL values from entering restored state.
 */
export function validateImportedValue(
  value: unknown,
  path = "package",
  depth = 0
): void {
  if (depth > 20) {
    throw new Error("Imported assessment package is too deeply nested.");
  }
  ...
  if (Array.isArray(value)) {
    ...
    value.forEach((item, index) =>
      validateImportedValue(item, `${path}[${index}]`, depth + 1)
    );
    return;
  }
...
/**
 * Assessment Profile boundary validation. The returned profile can be
 * safely placed into React state and subsequently exported.
 */
export function validateAssessmentProfileSecurity<T extends Record<string, unknown>>(
  profile: T
): T {
